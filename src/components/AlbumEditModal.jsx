...
import { faXmark, faPlus, faChevronUp, faChevronDown, faCamera, faCloudArrowUp, faTriangleExclamation, faCompactDisc } from '@fortawesome/free-solid-svg-icons';
...
function AlbumEditModal({ album, isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div 
        className={`relative z-50 bg-white dark:bg-gray-800 rounded-lg border-2 border-gray-200 dark:border-gray-700 shadow-2xl max-w-3xl w-full max-h-[90vh] flex flex-col overflow-hidden transform transition-all duration-300 ease-out ${isVisible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-8 scale-95'}`}
        onClick={(e) => e.stopPropagation()}
        onMouseDown={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 px-6 py-4 border-b border-gray-200 dark:border-gray-600 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <FontAwesomeIcon icon={faCompactDisc} className="w-5 h-5" />
            <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Edit Album</h2>
          </div>
          <button
            type="button"
            onClick={handleClose}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          <form id="album-edit-form" onSubmit={handleSubmit} className="space-y-4">
            {/* Cover Image Upload */}
        ...
        {/* Footer */}
        <div className="bg-gray-100 dark:bg-gray-700 border-t border-gray-200 dark:border-gray-600 px-6 py-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <button
            type="button"
            onClick={handleClose}
            className="px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-50 dark:hover:bg-gray-600 transition self-start"
          >
            Cancel
          </button>
          <button
            ...
            className="px-6 py-2 bg-primary-600 text-white rounded-md hover:bg-primary-700 transition self-end sm:self-auto"
          >
            Save Changes
          </button>
        </div>
